...
homicides_clean2 |>
  ggplot(mapping = aes(x = year, y = count, color = homicide_type)) +
  geom_point() +
  theme_minimal() +
...
base <- 
  homicides_clean2 |>
  ggplot(aes(x = year, y = count, fill = homicide_type )) +
  labs(
    x = "Year",
    y = "Homicide Count",
    color = "Homicide Type"
...
base_plot <- 
  homicides_clean2 |>
  ggplot(aes(x = year, y = count, colour = "Shooting")) +
  geom_point(alpha = 0.5) +
  labs(
    x = "Five-kilometer time (minutes)",
    y = "Marathon time (minutes)",
    colour = "Homicide Type"
  ) +
  theme_classic()
library(ggplot2)
```
